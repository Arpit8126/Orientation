-- =========================================================================
-- POOKIZ — Fix Google Auth Subdomain Matching & Domain Restriction
-- Run this in your Supabase SQL Editor to update your triggers.
-- =========================================================================
...
BEGIN
  ...
  -- Find matching university ID (matching exact domain OR subdomain like ds.study.iitm.ac.in matching study.iitm.ac.in)
  SELECT id, name INTO _university_id, _univ_name 
  FROM public.universities 
  WHERE LOWER(_domain) = LOWER(domain) 
     OR LOWER(_domain) LIKE '%.' || LOWER(domain)
  ORDER BY LENGTH(domain) DESC -- Prefer the most specific match first
  LIMIT 1;

  -- Block any emails that are not from allowed public domains or registered universities
  IF LOWER(_domain) NOT IN ('gmail.com', 'yahoo.com', 'outlook.com', 'hotmail.com') AND _university_id IS NULL THEN
    RAISE EXCEPTION 'Registration failed: Domain @% is not registered as a university or public email provider.', _domain;
  END IF;
